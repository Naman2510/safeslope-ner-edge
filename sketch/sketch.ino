...
struct SensorData {
  ...
  // Existing simulated hydrological values.
...
bool readMPU(
  ...
  // Skip MPU6050 temperature registers.
  Wire.read();
  Wire.read();
...
void updateSensors() {
  ...
  if (sensor.mpuOk) {
    // --------------------------------------------------------
    // Convert raw accelerometer readings to g.
    // --------------------------------------------------------
    ...
    // --------------------------------------------------------
    // Calculate pitch from the accelerometer.
    // --------------------------------------------------------
    ...
    // --------------------------------------------------------
    // Calculate roll from the accelerometer.
    // --------------------------------------------------------

    const float instantRoll = atan2f(
      sensor.accelXG,
      sensor.accelZG
    ) * 180.0f / PI;
    ...
    // --------------------------------------------------------
    // Preserve the existing three-sample pitch filter.
    // --------------------------------------------------------
    ...
    //
    // The accelerometer magnitude is approximately 1g
    // while the device is stationary because of gravity.
    // We subtract 1g and calculate the RMS of the remaining
    // movement component.
    // --------------------------------------------------------

    const float accelerationMagnitude = sqrtf(
      sensor.accelXG * sensor.accelXG +
      sensor.accelYG * sensor.accelYG +
      sensor.accelZG * sensor.accelZG
    );

    const float vibrationComponent =
      accelerationMagnitude - 1.0f;

    vibrationHistory[vibrationIndex] =
      vibrationComponent * vibrationComponent;

    vibrationIndex =
      (vibrationIndex + 1) % VIBRATION_WINDOW_SIZE;
    ...
    for (uint8_t i = 0; i < vibrationCount; i++) {
      vibrationSum += vibrationHistory[i];
    }

    sensor.vibrationRmsG =
      sqrtf(vibrationSum / vibrationCount);
  }

  // ----------------------------------------------------------
  // Existing simulated potentiometer inputs.
  // ----------------------------------------------------------

  sensor.moisturePct =
    adcPercent(PIN_SOIL_MOISTURE);

  sensor.poreKpa =
    adcPercent(PIN_PORE_PRESSURE);
}
